...
- Admin-Konten können im Admin-Tab (Bereich "Nutzer") weitere Nutzer anlegen — dabei wird nur Vorname/Nachname angegeben, kein Passwort. Der Nutzername (z.B. `max.muster`) wird automatisch generiert, bei Namensgleichheit mit angehängter Zahl. Der jeweilige Nutzer wählt sein Passwort selbst beim allerersten Login ("Passwort festlegen"-Formular erscheint automatisch).
...
Mehrere Vereins-Tools (Materialliste, TrainerCheckliste, Spielertool) speichern ihre eigenen Daten per WebDAV in derselben Nextcloud. Statt dort ein eigenes Formular mit WebDAV-Adresse, Benutzername und App-Passwort zu verlangen, nutzen sie dieselbe Anmeldung wie diese Übersicht:
